...
import com.mihir.alzheimerscaregiver.caretaker.data.repository.TaskRepository;
...
public class AddEditTaskActivity extends AppCompatActivity {
    ...
    private void populateFieldsWithExistingData() {
        ...
        // Basic fields
        inputTaskName.setText(existingTask.name);
        inputTaskDescription.setText(existingTask.description);
        inputTaskCategory.setText(existingTask.category);
        
        if (existingTask.scheduledTimeEpochMillis != null) {
            SimpleDateFormat timeFormat = new SimpleDateFormat("hh:mm a", Locale.getDefault());
            String timeString = timeFormat.format(new java.util.Date(existingTask.scheduledTimeEpochMillis));
            inputTaskTime.setText(timeString);
            selectedTime.setTimeInMillis(existingTask.scheduledTimeEpochMillis);
        }
        ...
        if (existingTask.isRepeating) {
            buttonSunday.setChecked(existingTask.repeatOnSunday);
            buttonMonday.setChecked(existingTask.repeatOnMonday);
            buttonTuesday.setChecked(existingTask.repeatOnTuesday);
            buttonWednesday.setChecked(existingTask.repeatOnWednesday);
            buttonThursday.setChecked(existingTask.repeatOnThursday);
            buttonFriday.setChecked(existingTask.repeatOnFriday);
            buttonSaturday.setChecked(existingTask.repeatOnSaturday);
        }

        // Notification settings
        checkEnableAlarm.setChecked(existingTask.enableAlarm);
        checkCaretakerNotification.setChecked(existingTask.enableCaretakerNotification);
    }
    ...
    private void saveTask() {
        ...
        if (isEditMode && existingTask != null) {
            ...
            task = new TaskEntity(taskName, taskDescription, false, category, null, false, null);
        }

        // Set scheduled time
        if (!inputTaskTime.getText().toString().trim().isEmpty()) {
            task.scheduledTimeEpochMillis = selectedTime.getTimeInMillis();
        }

        // Set repeat settings
        task.isRepeating = checkRepeating.isChecked();
        if (checkRepeating.isChecked()) {
            task.repeatOnSunday = buttonSunday.isChecked();
            task.repeatOnMonday = buttonMonday.isChecked();
            task.repeatOnTuesday = buttonTuesday.isChecked();
            task.repeatOnWednesday = buttonWednesday.isChecked();
            task.repeatOnThursday = buttonThursday.isChecked();
            task.repeatOnFriday = buttonFriday.isChecked();
            task.repeatOnSaturday = buttonSaturday.isChecked();
        }

        // Set notification settings
        task.enableAlarm = checkEnableAlarm.isChecked();
        task.enableCaretakerNotification = checkCaretakerNotification.isChecked();
